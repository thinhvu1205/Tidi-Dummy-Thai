    ...
    public void setCurentTurn(JObject data, string status)
    {
        ...
        */
        ...
        for (int i = 0; i < numberChip; i++)
        {
            ...
            if (chipEffectPool.Count < 1) chipEffectPool.Add(Instantiate(chipEffect, Table.transform));
    ...
    public void handleStartGame(JObject data)
    {
        ...
        for (int i = 0; i < players.Count; i++)
        {
            ...
            if (player.id == getInt(data, "idDealer"))
            {
                ...
                //    isLeft = true;
                //}
                player.playerView.showDealer(true, player.playerView.transform.localPosition.x>0, isUp);
            }
            else
            {
