...
public class GameView : BaseView
{
    ...
    protected override void Start()
    {
        ...
        string soundBg = SOUND_GAME.IN_GAME_COMMON;
        if (Config.curGameId == (int)GAMEID.SLOTNOEL)
        {
            soundBg = SOUND_SLOT.BG_NOEL;
        }
        else if (Config.curGameId == (int)GAMEID.SLOTTARZAN)
        {
            soundBg = SOUND_SLOT.BG_TARZAN;
        }
        else if (Config.curGameId == (int)GAMEID.SLOT_9900)
        {
            soundBg = SOUND_SLOT.BG_JUICY_GARDEN;
        }
        ...
        for (var i = 0; i < listPosView.Count; i++)
        {
            ...
            {
                btnInvite = Instantiate(invitePrefab, transform);
            }
            else
            {
                btnInvite = Instantiate(invitePrefab, inviteContainer);
            }
        ...
        if (txtGameName != null)
            txtGameName.text = Config.getTextConfig(Config.curGameId.ToString());
        DOTween.Sequence().AppendInterval(1.0f).AppendCallback(() =>
        {
            Debug.Log("getFarmInfo");
    ...
    {
        Globals.Config.isBackFromGame = true;
    }
    ...
    public void onClickBack()
    {
        ...
        var sizee2 = subView.transform.Find("Bkg").GetComponent<RectTransform>().sizeDelta;
        if (transform.eulerAngles.z == 0)
        {
            //subView.GetComponent<GroupMenuView>().setOriginPosition(-Screen.width * .5f + sizee2.x * .5f + 10f, Screen.height * .5f - sizee2.y * .5f - 10f);

            subView.GetComponent<GroupMenuView>().setOriginPosition(-720.0f * .5f + sizee2.x * .5f + 10f, 1280.0f * .5f - sizee2.y * .5f - 10f);
        }
        else
        {
            subView.GetComponent<GroupMenuView>().setOriginPosition(-1280.0f * .5f + sizee2.x * .5f + 10f, 720.0f * .5f - sizee2.y * .5f - 10f);
        }
        subView.transform.SetAsLastSibling();
        subView.GetComponent<GroupMenuView>().show();
    ...
    public void onClickInfoPlayer(Player player)
    {
        SoundManager.instance.soundClick();
        if (stateGame == STATE_GAME.VIEWING) return;
    ...
    public void handleChatTable(JObject data)
    {
        ...
        if (player != null)
        {
            if (datType.Contains("*f") && !datNName.Equals(""))
            {
                ...
                if (npl != null)
                {
                    //var idAnimation = int.Parse(datType.Substring(datType.IndexOf('f') + 1, datType.Length));
                    ...
                    iteChat.setData(idAnimation, false);
                    DOTween.Sequence().Append(iteChat.transform.DOMove(posTo, 1).SetEase(Ease.OutQuad)).AppendCallback(() =>
                    {
                        iteChat.setData(idAnimation, true);
                    });
                }
            }
            else
    ...
    public virtual void handleLTable(JObject data)
    {
        ...
        else
        {
            ...
            if (dataLeave != null)
            {
                ...
                Debug.Log("handleLTable:emitSIOWithValue=" + dataLeave.ToString());
            }
            dataLeave = null;
            Destroy(gameObject);
    ...
    public virtual void handleSTable(string strData)
    {
        ...
        //cc.NGWlog('===========> handleSTable: ');
        for (var i = 0; i < players.Count; i++)
        {
            // playerViewPool.put(players[i]._playerView.node);
            //players[i]._playerView.node.destroy();
    ...
    public virtual void onLeave()
    {
        ...
        if (stateGame != STATE_GAME.PLAYING)
        {
            ...
            //	NetworkManager.getInstance().isConnectSimilar = true;
            User.userMain.lastGameID = 0;
            //	//require('UIManager').instance.onShowLoad();
            //	cc.NGWlog("data levave game viwe la== " + JSON.stringify(dataLeave))
            //	if (typeof dataLeave != 'undefined') {
            //		require('SMLSocketIO').getInstance().emitSIOWithValue(dataLeave, "LeavePacket", false);
            //	}
            cleanGame();
            if (thisPlayer != null)
    ...
    public virtual void setGameInfo(int m, int id = 0, int maxBet = 0)
    {

        Config.tableMark = m;
        agTable = m;
        if (Config.curGameId == (int)GAMEID.BACCARAT || Config.curGameId == (int)GAMEID.BLACKJACK)
        {
    ...
    protected Player getPlayer(string namePlayer, bool isGetWithDisplayName = false) // mot so game n lai tra data name theo display name a son ak;
    {
        ...
        //playerFind = players.FirstOrDefault(pl => pl.namePl.Equals(namePlayer));
        //if (playerFind == null)
        //{
        //    playerFind = players.FirstOrDefault(pl => pl.displayName.Equals(namePlayer));
        //}
        for (int i = 0; i < players.Count; i++)
        {
            PlayerView playerV = (PlayerView)players[i].playerView;
            ...
            if (players[i].namePl.Equals(namePlayer) || displayName.Equals(namePlayer) || playerV.txtName.text.Equals(namePlayer))
            {
    ...
    protected virtual void updatePositionPlayerView()
    {
        for (var i = 0; i < players.Count; i++)
        {
            ...
            //if (Config.curGameId == (int)GAMEID.TONGITS11)//tam bo da
            //    players[i]._playerView.setPosThanhBarThisPlayer();
        }
        //if (Config.curGameId == (int)GAMEID.TONGITS)
    ...
    {
        var plView = Instantiate(playerViewPrefab, transform);//.GetComponent<PlayerView>();
        ...
        switch (Config.curGameId)
        {
            ...
            case (int)GAMEID.BORKDENG:
                {
                    return plView.GetComponent<PlayerViewBorkKDeng>();
                }
    ...
    {
        Debug.Log("(Config.dataVipFarm:" + Config.dataVipFarm);

        if (Config.dataVipFarm != null)
        {
