...
public class ChatIngameView : BaseView
{
    ...
    protected override void Awake()
    {
        ...
        for (var i = 1; i <= 10; i++)
        {
            string msg = Globals.Config.getTextConfig(string.Format("chat_text_{0}_{1}", Globals.Config.curGameId, i));
        ...
        for (var i = 1; i <= 9; i++)
        {
            ...
            item.GetComponent<Button>().onClick.AddListener(() =>
            {
                onClickChatEmo(iidd, 1);
            });

            var skeEmo = Resources.Load<SkeletonDataAsset>("GameView/Emo/fish/" + i + "/skeleton_SkeletonData");
    ...
    void onClickChatEmo(string id, int type)
    {
        Globals.Logging.Log("onClickChatEmo: " + id + " type " + type);
        SocketSend.sendChatEmo(Globals.User.userMain.displayName, "", (type == 1 ? "*e" : "*f") + id);
        hide();
    }
